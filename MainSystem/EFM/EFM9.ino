...
void setup() {
    ...
    CanSatLogData.flush();
    SensorData.println("gps_time,gps_latitudeitude,gps_longitude,gps_velovity,Temperature,Pressure,accelX,accelY,accelZ,magX,magY,R,G,B,ultra_distance");
    SensorData.flush();
 ...
 if (Serial1.available() > 0) {//これないとGPS使えない
   ...
   if (gps.location.isUpdated()) {//この中でgpsのデータを使わないと，うまく値が表示されない
    if (timeCounter1 > 0) {
        ...
        SensorData.print(",");
        SensorData.print(magX);
        SensorData.print(",");
        SensorData.print(magY);
        SensorData.print(",");
        SensorData.print(R);
        SensorData.print(",");
        SensorData.print(G);
